...
//		log.info(curentAccount.toString());
    ...
    @Override
    public Account UpdateAccountWithPermission(Account account) {
        Account getAccount = accountRepo.getOne(account.getId());
        if (getAccount != null) {
//            getAccount.setAuthrority(account.getAuthrority());
            getAccount.setAuthrority(Validator.isNotNull(account.getAuthrority()) ? account.getAuthrority() : new HashSet<Authrority>() {
                private static final long serialVersionUID = 1L;
                {
                    Authrority authro = new Authrority();
                    authro.setName("USER");
           ...
           return  accountRepo.save(getAccount);

        }
        throw new IndexOutOfBoundsException("không tồn tại idAccount");
...
//		log.info("đã tìm được user có email là" + email);
    ...
    @Override
    public Optional<Account> getUserWithAuthorities(Integer accountID) {
        // TODO Auto-generated method stub
...
//	}
    ...
    @Override
    public List<Account> getPage(int page, int limit, String sortBy, String order) {
        Direction directtion = Direction.valueOf(order);
